...
const Dashboard = (function () {
    ...
    function renderYourAnglerCard(state) {
        ...
        if (bookedLake) {
            ...
            html += '<div style="display:flex;gap:0.9rem;align-items:flex-start;">';
            html += '<img src="data:image/svg+xml;utf8,' + encodeURIComponent('<svg xmlns=\"http://www.w3.org/2000/svg\" width=\"220\" height=\"120\" viewBox=\"0 0 220 120\"><rect width=\"220\" height=\"120\" fill=\"#1b3a26\"/><rect x=\"0\" y=\"0\" width=\"220\" height=\"120\" fill=\"none\" stroke=\"rgba(255,255,255,0.15)\" stroke-width=\"2\"/><text x=\"110\" y=\"62\" font-family=\"sans-serif\" font-size=\"12\" fill=\"rgba(255,255,255,0.6)\" text-anchor=\"middle\">No lake booked</text></svg>') + '" alt="No booking" style="width:7.5rem;height:10rem;border-radius:8px;border:1px solid var(--colour-border);object-fit:cover;flex:0 0 auto;" onerror="this.style.display=\'none\'" />';
            html += '<div style="flex:1 1 0%;min-width:0;">';
            html += '<div style="font-weight:800;color:var(--colour-text-muted);">Not booked</div>';
            html += '<div style="font-size:0.8rem;color:var(--colour-text-muted);margin-top:0.2rem;">Browse lakes to book a session</div>';
            html += '</div>';
            html += '</div>';
        }
        html += '</div>';
    ...
    function renderAnglerQuestsCard(state) {
        ...
        if (total === 0) {
            ...
            html += '<div style="max-height:18rem;overflow-y:auto;padding-right:0.2rem;">';
    ...
    function renderAnglerPBCard(state) {
        ...
        var speciesMax = typeof Fish !== 'undefined' && pbFish && Fish.SPECIES[pbFish.species] ? Fish.SPECIES[pbFish.species].maxWeight : 1200;
        var weightPct = Math.min(100, Math.round((pbWeight / speciesMax) * 100));
        var RARITY_ORDER = ['mythic','legendary','epic','rare','uncommon','common'];
        var rarityIdx = RARITY_ORDER.indexOf((pbFish && pbFish.rarity) ? pbFish.rarity : 'common');
        var rarityPct = rarityIdx === -1 ? 0 : Math.round(((RARITY_ORDER.length - 1 - rarityIdx) / (RARITY_ORDER.length - 1)) * 100);
        var health = pbFish && pbFish.stats ? (pbFish.stats.health || 0) : 0;
        var age = pbFish ? (pbFish.age_days || 0) : 0;
        var maxAge = 2000;
        var agePct = Math.min(100, Math.round((age / maxAge) * 100));
        var alive = state.fish ? state.fish.filter(function(f){ return f.alive; }) : [];
        var totalValue = typeof Fish !== 'undefined' ? Fish.getTotalStockValue(alive) : 0;
    ...
    function renderRarestFishCardSpotlight(state) {
        ...
        alive.forEach(function(f){
            ...
            var sameTierBetter = (idx === bestRarityIdx && rarest && Fish.getFishValue(f) > Fish.getFishValue(rarest));
            if (betterTier || sameTierBetter) { bestRarityIdx = idx; rarest = f; }
        });
        if (!rarest) return '<p class="empty-state">No fish yet.</p>';
        ...
        html += '<span class="fish-spotlight-stat">❤️ Health:\u00A0' + (rarest.stats ? rarest.stats.health : 0) + '</span>';
        html += '<span class="fish-spotlight-stat">📅 Age:\u00A0' + (rarest.age_days || 0) + 'd</span>';
    ...
    function renderFishTracker(state) {
        ...
        if (log.length === 0) {
            html += '<p class="empty-state">No fish recorded yet. Buy from the Shop, breed, or claim Oakmere Lake to get started.</p>';
        } else {
